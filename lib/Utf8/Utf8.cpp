...
std::string utf8ComposeNfc(const std::string& in) {
  ...
  // Hangul jamo (lead byte 0xE1). Plain ASCII and already-precomposed text -- the
  // vast majority of names -- have none, so return them untouched without allocating.
...
void utf8ComposeNfcInPlace(char* buffer) {
  ...
  while (*read) {
    ...
    if (composed) {
      // Supported compositions are BMP codepoints and fit within the consumed
      // pair's bytes, so rewriting the base cannot overtake the read cursor.
      write = baseStart;
      if (composed < 0x800) {
      ...
      *write++ = static_cast<char>(0x80 | (composed & 0x3F));
      ...
      baseStart = write;
      const size_t length = static_cast<size_t>(read - start);
      // Preserve uncomposed bytes, including malformed UTF-8: replacement
      // characters could expand the caller's fixed-size display buffer.
