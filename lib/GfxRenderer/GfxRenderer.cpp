...
uint16_t getSdCardSpaceAdvance(SdCardFont& font, const EpdFontFamily::Style style) {
  ...
  // A zero advance-table result can mean the space was not cached. Fall back
  // to the font's ordinary glyph lookup instead of collapsing word spacing.
...
void GfxRenderer::preserveImagePolarity(const int x, const int y, const int width, const int height) const {
  ...
  // drawBitmap writes the normal BW polarity into the framebuffer. In an
  // inverted display frame, flip only the image rectangle back so pictures
  // retain their source polarity while surrounding text/UI stays inverted.
...
void GfxRenderer::releaseFrameBufferForBuild() {
  ...
  frameBuffer = nullptr;
  if (scratch != nullptr) buildscratch::lend(scratch, size);
}
...
void GfxRenderer::FrameBufferLoan::end() {
  ...
  if (!renderer_.restoreFrameBufferAfterBuild()) {
    LOG_ERR("GFX", "Framebuffer restore failed");
    ESP.restart();
  }
}
...
void GfxRenderer::tapToLogical(float nx, float ny, int& outX, int& outY) const {
  ...
  phyX = std::clamp(phyX, 0, static_cast<int>(panelWidth) - 1);
  phyY = std::clamp(phyY, 0, static_cast<int>(panelHeight) - 1);
  switch (orientation) {
    case Portrait:
