...
bool coverCacheValid(const std::string& bookDir) {
  HalFile file;
  uint8_t header[26] = {};
  const std::string path = coverPath(bookDir);
  if (!Storage.openFileForRead("WR", path, file) || file.fileSize64() < sizeof(header) ||
  ...
  const int32_t height = static_cast<int32_t>(readLe32(header + 22));
  const uint32_t absoluteHeight = height < 0 ? static_cast<uint32_t>(-static_cast<int64_t>(height))
                                             : static_cast<uint32_t>(height);
  return width == kCoverThumbWidth && absoluteHeight == kCoverThumbHeight;
}
