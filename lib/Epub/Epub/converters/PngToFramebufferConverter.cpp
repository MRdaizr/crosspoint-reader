...
bool pngHeapAvailable(const char* what) {
  ...
  if (maxBlock >= PNG_DECODER_APPROX_SIZE &&
      freeHeap >= PNG_DECODER_APPROX_SIZE + PNG_DECODE_HEAP_SLACK) {
    return true;
  }
  LOG_ERR("PNG", "Not enough heap for PNG %s (free=%u maxAlloc=%u; need %u contiguous, %u free)", what,
          ...
          static_cast<unsigned>(PNG_DECODER_APPROX_SIZE),
          static_cast<unsigned>(PNG_DECODER_APPROX_SIZE + PNG_DECODE_HEAP_SLACK));
  return false;
}
