...
parser.add_argument("--mono", dest="mono", action="store_true", help="For 1-bit fonts, rasterise with FreeType's native monochrome renderer instead of antialiased-greyscale then threshold. Ignored with --2bit.")
...
if useMono:
    # Native monochrome rasterisation produces crisper, more consistent small
    # UI glyphs. It does not alter glyph metrics or layout.
...
for i_start, i_end in intervals:
    for code_point in range(i_start, i_end + 1):
        ...
        if useMono:
            # FreeType's monochrome bitmap is row-padded and MSB-first;
            # repack it into the firmware's continuous 1-bit stream.
            pixelsbw = []
            px = 0
