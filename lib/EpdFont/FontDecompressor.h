...
class FontDecompressor {
 ...
 private:
  ...
  // failing. ensureCapacity() returns false on OOM so the caller can skip the glyph gracefully.
  ...
  // Grow (never shrink) an owned buffer to at least `needed` bytes; false on OOM.
