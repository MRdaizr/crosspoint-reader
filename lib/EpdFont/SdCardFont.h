...
// macro (rather than a constexpr) so external SD-font release tooling can
// stringify it into a manifest URL. No integer suffix because stringification
// would include it (e.g. `4U` → `"4U"`).
//
...
class SdCardFont {
 public:
  ...
  // Returns a bitmap only when the glyph belongs to the currently active mini
  // cache and its offset/length are inside the resident bitmap arena. This
  // prevents a stale metadata-only glyph from becoming an invalid load.
  const uint8_t* getResidentBitmap(const EpdFontData* data, const EpdGlyph* glyph) const;
 ...
 private:
  ...
  // CJK books exhaust 768 unique characters within a few dozen pages. Once
  // full, layout falls back to repeated on-demand SD glyph reads. 2048 keeps
  // the persistent metric table compact while covering typical reading runs.
