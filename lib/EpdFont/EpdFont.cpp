...
int8_t EpdFont::getKerning(const uint32_t leftCp, const uint32_t rightCp) const {
  ...
  // Built-in fonts use split arrays; SD-card fonts retain packed class maps.
  const bool split = data->kernLeftCodepoints != nullptr;
  const uint8_t lc = split
                         ? lookupKernClassSplit(data->kernLeftCodepoints, data->kernLeftClassIds,
                                                data->kernLeftEntryCount, leftCp)
                         : lookupKernClass(data->kernLeftClasses, data->kernLeftEntryCount, leftCp);
  if (lc == 0) return 0;
  const uint8_t rc = split
                         ? lookupKernClassSplit(data->kernRightCodepoints, data->kernRightClassIds,
...
bool EpdFont::hasCodepoint(const uint32_t cp) const {
  const int count = data->intervalCount;
  if (count > 0 && data->intervals != nullptr) {
    const EpdUnicodeInterval* intervals = data->intervals;
    const auto* end = intervals + count;
    const auto it = std::upper_bound(
        intervals, end, cp, [](uint32_t value, const EpdUnicodeInterval& interval) { return value < interval.first; });
    if (it != intervals && cp <= (it - 1)->last) {
      return true;
    }
  }

  // SD-card fonts expose their complete coverage index through this callback,
  // while keeping only the current page's glyphs in data->intervals.
  return data->coverageHandler != nullptr && data->coverageHandler(data->glyphMissCtx, cp);
}
