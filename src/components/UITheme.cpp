...
#include "components/themes/roundedraff/RoundedRaffExtTheme.h"
...
void UITheme::setTheme(CrossPointSettings::UI_THEME type) {
  switch (type) {
    ...
    case CrossPointSettings::UI_THEME::ROUNDEDRAFF:
      ...
      break;
    case CrossPointSettings::UI_THEME::ROUNDEDRAFF_EXT:
      LOG_DBG("UI", "Using RoundedRaffExt theme");
      currentTheme = std::make_unique<RoundedRaffExtTheme>();
      currentMetrics = &RoundedRaffMetrics::values;
      break;
...
void UITheme::drawCenteredWrappedText(const GfxRenderer& renderer, Rect screen, const int fontId, const char* text,
  ...
  if (verticalAlignment == TextVerticalAlignment::CENTER) {
    y += std::max(0, (screen.height - totalHeight) / 2);
  }
