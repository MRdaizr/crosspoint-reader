...
void Lyra3CoversTheme::drawRecentBookCover(GfxRenderer& renderer, Rect rect, const std::vector<RecentBook>& recentBooks,
  ...
  if (hasContinueReading) {
    ...
    for (int i = 0; i < std::min(static_cast<int>(recentBooks.size()), Lyra3CoversMetrics::values.homeRecentBooksCount);
      ...
      std::string titleText = recentBooks[i].title;
      titleText += "\n\xe2\x80\xa6";
      const int titleFontId = DynamicFont::fontForCjkText(renderer, recentBooks[i].title.c_str(), SMALL_FONT_ID);
      DynamicFont::prewarmIfSdFont(renderer, titleFontId, titleText);
      auto titleLines = renderer.wrappedText(titleFontId, recentBooks[i].title.c_str(), maxLineWidth, 3);
