...
void discover(std::vector<DictionaryEntry>& out) {
  ...
  for (const char* dictRoot : DICT_ROOTS) {
    ...
    for (auto entry = rootDir.openNextFile(); entry; entry = rootDir.openNextFile()) {
      ...
      // CrossPointSettings stores the selected folder in a fixed 31-byte
      // field. Do not expose a folder that would be silently truncated and
      // therefore impossible to resolve on the next lookup.
      if (strlen(name) >= 32) {
        LOG_DBG("DREG", "Skipping dictionary folder with oversized name: %s", name);
        continue;
      }
