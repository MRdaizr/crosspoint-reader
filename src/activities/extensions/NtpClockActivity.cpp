...
void NtpClockActivity::render(RenderLock&&) {
  ...
  if (state == State::CONNECTING || state == State::SYNCING) {
    ...
    BigClock::drawTime(renderer, pageWidth, pageHeight, localTime.tm_hour, localTime.tm_min);
    const int clockY = BigClock::top(pageHeight);
    ...
    renderer.drawCenteredText(dateFont, clockY + BigClock::DIGIT_HEIGHT + 32, dateText);
    if (lastSyncFailed) {
      renderer.drawCenteredText(UI_10_FONT_ID, clockY + BigClock::DIGIT_HEIGHT + 62, tr(STR_NTP_CLOCK_USING_RTC));
    }
  } else {
