...
void ReaderToolbarUi::buildToolRow(UiScreen& screen, const fui::LayoutAnchor anchor) {
  ...
  const FuiMenuIconSlot slots[kToolCount] = {FuiMenuIconSlot::ReaderToolbarContents,
                                             FuiMenuIconSlot::ReaderToolbarText,
                                             FuiMenuIconSlot::ReaderToolbarMore};
  ...
  for (int i = 0; i < kToolCount; ++i) {
    ...
    if (GUI.showsFuiMenuIcon(slots[i])) {
      ...
      auto textStyle = tokens.bodyText;
      textStyle.align = fui::TextAlign::Center;
      textStyle.bold = i == model_.activeTool;
      screen.target().text(slot.inset(fui::Insets{4, 4, 4, 4}), labels[i], textStyle);
    }
...
void ReaderToolbarUi::buildToolbar(UiScreen& screen) {
  ...
  // Scrub row: < [progress track + knob: tap/drag to jump] >
  {
    ...
    stepProps_.label = GUI.showsFuiMenuIcon(FuiMenuIconSlot::ReaderToolbarPrevChapter) ? nullptr : tr(STR_DIR_LEFT);
    stepProps_.icon = fui::bitmapFromIcon(icon_reader_back_24);
    if (stepProps_.label) stepProps_.icon = {};
    ...
    stepProps_.label = GUI.showsFuiMenuIcon(FuiMenuIconSlot::ReaderToolbarNextChapter) ? nullptr : tr(STR_DIR_RIGHT);
