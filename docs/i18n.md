...
The built-in UI font set is intentionally limited to English, Simplified
Chinese and Japanese. Book content may use the SD-card font fallback for
additional characters. Japanese UI translations are kept in
`lib/I18n/translations/japanese.yaml`; any untranslated string falls back to
English.
...
`"EN"`, `"ZH_CN"`, or `"JA"` rather than a raw enum value.
