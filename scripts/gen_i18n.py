...
def generate_keys_header(
    ...
    # If a legacy language is no longer built into the firmware, map it to EN.
    # This keeps old language.bin migration valid for size-reduced builds.
    v1_codes = [
        "EN", "ES", "FR", "DE", "CS", "PT", "RU", "SV", "RO", "CA", "UK",
        "BE", "IT", "PL", "FI", "DA", "NL", "TR", "KK", "HU", "LT", "SI",
    ]
    v1_mapped_codes = [c if c in languages else "EN" for c in v1_codes]
    lines.append("// V1 language.bin migration table (frozen enum order from 2f969a9)")
    lines.append("constexpr Language V1_LANGUAGES[] = {")
    lines.append("    " + ", ".join(f"Language::{c}" for c in v1_mapped_codes) + ",")
    lines.append("};")
    lines.append(
